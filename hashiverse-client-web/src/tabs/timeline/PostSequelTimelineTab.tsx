...
import banner_sequel from "../../media/banner_sequel.svg";
...
export const PostSequelTimelineTab: React.FC<Props> = (props) => {
	const { post_id, bucket_location } = useParams();
	if (!post_id || !bucket_location) return <ErrorTab />;
	return <PostSequelTimelineTabInner {...props} post_id={post_id} bucket_location={bucket_location} />;
};

const PostSequelTimelineTabInner: React.FC<Props & { post_id: string; bucket_location: string }> = ({ hashiverse, user_settings_cache, post_id, bucket_location }) => {
	...
	const header = (
		<>
			<Banner
				image={<img src={banner_sequel} alt="sequel" />}
				heading={
					<Text size="xl" fw={700}>
						{t("banner.sequels_to_post")}
					</Text>
				}
				detail={<Text>{t("banner.sequels_detail")}</Text>}
			/>
			{post ? (
	...
	return (
		<div className="FullColumnChildAndParent">
			...
			<TimelineControl
				...
				timeline_key={`sequel:${post_id}`}
				get_more={() => hashiverse.single_timeline_get_more_sequel_v1(post_id)}
