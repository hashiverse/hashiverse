...
const prompt = "You are updating translations for the hashiverse www/ site. Run `node www/translations/check-translations.mjs` from the repo root to (re)generate this JSON — its stdout is exactly the {prompt, summary, actions_required} structure you are reading, with a fresh actions_required block. Exit code is 1 while any work remains and 0 once everything is registered fresh. For every entry under actions_required, do the following based on the source path: (a) for `src/pages/en/**/*.astro` — read the English file, translate into the target language preserving imports, JSX tags, class names, and code blocks, write to `src/pages/<lang>/<same-relative-path>` (swap `/en/` for `/<lang>/`). (b) for `src/i18n/en.json` — read it, translate each string value into the target language, write to `src/i18n/<lang>.json`. Do NOT translate the product name (Hashiverse) or technical/cryptographic terms (e.g. Kademlia, Ed25519, P2P, hashtag). After translating, also delete any files listed under actions_required[lang].delete. Then update `www/translations/state.json` with the new source hashes (set state[lang][src_path] = the SHA-256 of the current English source bytes for every source you translated) and re-run the same command to confirm exit 0.";
