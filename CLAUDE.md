...
cargo nextest run -p hashiverse-integration-tests  # Run this one only for major refactors as it takes forever.

# Run a single test by name
cargo nextest run -p hashiverse-lib <test_name>

# Run WASM tests for hashiverse-lib
cargo nextest run --target wasm32-wasip1 -p hashiverse-lib

# Run WASM client tests in headless Chrome (requires wasm-pack)
